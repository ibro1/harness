...
function createExecutor(
  ...
  return async (args: unknown, exec: ToolExecution) => {
    ...
    // isError, logs at info. `options.name` already carries the server, so the
    // line names both halves without this executor holding the server config.
    ...
    try {
      ...
      // timeout — not a tool-reported isError (which set 'error' below).
      if (outcome === 'ok') outcome = 'failed'
      throw error
    } finally {
      const line = `mcp-client: ${options.name} → ${outcome} in ${String(Date.now() - startedAt)}ms`
