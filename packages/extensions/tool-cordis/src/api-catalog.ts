...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  {
    key: 'browserBridge',
    summary: 'The connected browser, and the request/reply channel to it.',
    description: 'The connected browser, and the request/reply channel to it.\n\nOne connection at a time: a second extension replaces the first, because two browsers answering the same `browser_click` would make the result ambiguous rather than doubling the capability.',
    methods: [
      {
        signature: 'profiles(): string[]',
        description: 'The profile labels with a live connection, sorted for stable output.',
        parameters: [],
        returns: 'every connected profile\'s label.',
      },
      {
        signature: 'async call(type: string, payload: unknown, profile?: string): Promise<unknown>',
        description: 'Send one command to the connected browser and await its reply.',
        parameters: [{ name: 'type', description: 'command name the extension dispatches on.' }, { name: 'payload', description: 'command arguments, JSON-serializable.' }, { name: 'profile', description: 'label of the browser to address; omitted when exactly one is connected.' }],
  ...
  {
    key: 'outputs',
    summary: 'The session-outputs capability registered on `ctx.outputs`.',
    description: 'The session-outputs capability registered on `ctx.outputs`.\n\nEvery method takes the session\'s working directory explicitly: the service has no ambient notion of "the current session", so the caller stays responsible for reading an authoritative cwd (the session store) rather than one a model supplied.',
    methods: [
      {
        signature: 'dir(cwd: string): string',
        description: 'Where a session\'s published outputs live: `<cwd>/.outputs`.',
        parameters: [{ name: 'cwd', description: 'the session\'s working directory.' }],
        returns: 'the absolute outputs directory path, whether or not it exists yet.',
      },
      {
        signature: 'publish(cwd: string, absPath: string, label?: string): Promise<PublishedOutput>',
        description: 'Copy a finished file into the session\'s outputs directory.\n\nThe source is copied, never moved and never symlinked: the producer may still be writing beside it, and a link into a temp directory is a dead link an hour later. An existing name is suffixed rather than overwritten.',
        parameters: [{ name: 'cwd', description: 'the session\'s working directory; the outputs directory is created under it.' }, { name: 'absPath', description: 'the finished file, absolute or resolved against `cwd`.' }, { name: 'label', description: 'optional free-text label from the publisher; it never influences the file name.' }],
        returns: 'the published copy, including the name actually used.',
        throws: ['when the cwd does not exist, the source is missing or is not a regular file, the source resolves outside the cwd and `allowOutsideCwd` is false, the source is already inside the outputs directory, or either byte cap would be exceeded. Nothing is copied when a publish is refused.'],
      },
      {
        signature: 'list(cwd: string): Promise<PublishedOutput[]>',
        description: 'What has been published for this session, newest first.',
        parameters: [{ name: 'cwd', description: 'the session\'s working directory.' }],
  ...
  {
    key: 'social',
    summary: 'Registry of social-posting providers and the router in front of them.',
    description: 'Registry of social-posting providers and the router in front of them.\n\n`register()` files each provider into the calling context\'s fiber, so a disposed provider plugin removes its targets from every later listing. Reads re-ask every provider: `ready` is a live fact about a credential, and a cached "ready" would publish under someone\'s name on the strength of a stale observation.',
    methods: [
      {
        signature: 'register(provider: SocialProvider): () => void',
        description: 'Register one borrowed same-process provider. The name must be unique and free of `:` and whitespace, because it is the prefix that makes every target id this provider lists resolve to exactly this registration.',
        parameters: [{ name: 'provider', description: 'the platform implementation to register.' }],
        returns: 'the exact Cordis effect disposer that unregisters it; disposing the registering fiber does the same.',
      },
      {
        signature: 'async targets(): Promise<readonly SocialTarget[]>',
        description: 'Every target across every registered provider, ready or not.',
        parameters: [],
        returns: 'the merged targets, ordered by id so the catalog is stable.',
      },
      {
        signature: 'async post(request: SocialPostRequest): Promise<SocialPostResult>',
        description: 'Publish one post through the provider owning `request.target`.\n\nEvery refusal happens here, before the provider is called: an unknown id lists the ids that exist, an unready target carries the provider\'s own reason, and an attachment or a body the target does not accept fails naming the target rather than surfacing a platform error from inside a provider.',
        parameters: [{ name: 'request', description: 'the target id, the text to publish verbatim, and any attachments.' }],
        returns: 'what the owning provider created.',
        throws: ['when the target is unknown, unready, or does not accept what the request carries.'],
      },
    ],
  },
  {
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'registerSignedInTarget(resolve: () => string): () => void',
        description: 'Claim where a successful sign-in on the password gate lands. A second browser gate mounted above this package would otherwise answer its own 401 to someone who just signed in here; the composing application registers a target that clears both in one redirect. One owner, last registration wins.',
        parameters: [{ name: 'resolve', description: 'computes the redirect target at sign-in time.' }],
