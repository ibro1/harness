...
async function setup(
  ...
  roots.push(ctx)
  if (transport) {
    // The fork's password gate belongs to the browser-facing realm; this
    // transport fixture authenticates its callers through the Gateway itself.
    await ctx.plugin(WebServer, { host: '127.0.0.1', port: 0, authenticate: false })
