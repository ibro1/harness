...
describe('registration', () => {
  ...
  it('unregisters everything on fiber disposal (HMR safety)', async () => {
    ...
    expect(sectionNames(await ctx.systemPrompt.assemble())).toEqual([
      ...
      'harness:identity', 'harness:turn-contract', 'tool:edit', 'tool:read', 'tool:write',
    ])
    await fiber.dispose()
    expect(ctx.tools.schemas()).toHaveLength(0)
    // Only the system-prompt plugin's own built-in sections remain.
    expect(sectionNames(await ctx.systemPrompt.assemble())).toEqual(['deployment:persona-prefix', 'deployment:persona-suffix', 'harness:identity', 'harness:turn-contract'])
  })
})
...
// Pinned verbatim: model-visible text, where a silent reword is exactly the
// kind of drift a behavioural rule cannot afford.
...
/** Preserve the default openers and exact section separators in the oracle. */
function withPersona(...sections: string[]): string {
  return ['You are an AI agent powered by DeepSeek Harness.', TURN_CONTRACT, ...sections].join('\n\n')
}
