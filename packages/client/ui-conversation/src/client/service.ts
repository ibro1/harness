...
export class ConversationController extends Service implements IConversation {
  ...
  addImagesFromFiles(actx: Context, files: readonly File[]): string | null {
    const sessionId = this.requireSessions().scopeOf(actx)
    if (sessionId === undefined) {
      throw new Error('conversation.addImagesFromFiles requires a session scope'
        + ' — address one via ctx.sessions.scope(id).conversation')
    }
    try {
      const drafts = this.createDrafts(sessionId, files)
      if (!this.input.for(actx).addAttachments(drafts.map(draft => draft.id))) {
