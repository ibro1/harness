...
- **Disconnect, per provider, behind a confirmation.** The confirmation names the provider, and names the other providers when the host reports that one credential record backs several (Meta's one record authorizes both `facebook` and `instagram`).
- **Three disconnect states, not two.** A button appears only when a record is actually stored. A provider nobody has signed in to reads `Not connected`, and one holding a working credential that the host cannot address gets the composition-gap sentence. Those last two were one state until they were not: a declared `credentialKeys` entry resolves an address for a provider with nothing behind it, so a fresh deployment was offered a button that would remove nothing — while every provider without a declared entry borrowed a sentence about misconfiguration to say "you have not signed in yet".
