...
`ctx.connection.reconnect()` interrupts active work, resets the sequence, and starts retry 1 immediately. Browser `offline` aborts active work, publishes `disconnected`, and suspends automatic attempts; the next `online` transition resets the sequence and starts at the 500ms tier. A tab returning to view — `visibilitychange` to visible, or the `pageshow` that is all a back/forward-cache restore fires — reconnects immediately when the connection is not already live, and does nothing when it is. That case is not reachable through `online`/`offline`: the network never went down, the tab did, and a hidden tab has its timers clamped and eventually frozen, so the retry that would have recovered the socket is not late but stopped. Sends continue to reach the Host over HTTP throughout, which is why a message sent from a disconnected-looking page is already answered once the page is reloaded by hand. Only a ready item publishes `connected`. Gateway mux owns no independent retry schedule.
