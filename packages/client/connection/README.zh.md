...
`ctx.connection.reconnect()` 会中断活动工作、重置序列，并立即开始 retry 1。浏览器 `offline` 会中断活动工作、发布 `disconnected` 并暂停自动尝试；下一次 `online` 转换会重置序列并从 500ms 档开始。标签页重新进入视图——`visibilitychange` 变为 visible，或者 back/forward 缓存恢复时唯一会触发的 `pageshow`——在连接尚未处于活跃状态时立即重连，已经活跃时则什么都不做。这种情形无法通过 `online`/`offline` 覆盖：掉下去的不是网络而是标签页，而隐藏的标签页其定时器会被限速并最终冻结，因此本该恢复该 socket 的那次重试不是迟到，而是停了。整个过程中发送仍然通过 HTTP 到达 Host，这正是为什么从一个看起来已断开的页面发出的消息，在手动刷新页面后已经被处理完毕。只有 ready 项会发布 `connected`。Gateway mux 不拥有独立重试调度。
