...
/**
 * The seats this plugin fills for a loopback browser (slot name → expected
 * components, in registration order). `settings.section` is a list this
 * package seats twice: General and Account.
 */
const SEATS = [
  ...
  ['settings.section', [GeneralSection, AccountSection]],
] as const
...
describe('ui-settings-general apply', () => {
  ...
  it('withholds the Host document action off-loopback', async ({ mock, start }) => {
    ...
    // The document action is the only thing loopback gates here. This fork
    // serves the UI behind its own password gate, so settings persist to the
    // Harness home off-loopback too: the describe read happens and the Host
    // preference wins over the browser language.
