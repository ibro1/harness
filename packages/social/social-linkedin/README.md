...
The application's own credentials come from whichever layer supplies them, checked in one order: the `social-linkedin` settings section this plugin serves, then composition config, then the environment variable named by `clientIdRef` (`LINKEDIN_CLIENT_ID` by default). Settings wins because it is the layer a person can change without a redeploy. The **client secret** is the exception and is never a value in any of those layers: `clientSecretRef` names the environment variable or credential record holding it, and it is resolved through the credential seam on each sign-in, so it is never written into a settings document and never read back to a browser. `redirectUri` follows the same settings-first order and must be the URI registered on the LinkedIn application; LinkedIn compares it byte-for-byte between the two OAuth legs, and sign-in refuses to start while no layer supplies one.
