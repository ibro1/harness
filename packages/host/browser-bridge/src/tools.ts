...
export function registerBrowserTools(ctx: Context, bridge: BrowserTarget): void {
  for (const tool of buildBrowserTools(bridge)) {
    ctx.effect(() => ctx.tools.register(tool), `browser-bridge: ${tool.name}`)
  }
}

/**
 * Build the browser tools without registering them, so one definition serves
 * both the harness's registry and the MCP catalogue a CLI reads. Two lists
 * would drift, and a drifted schema is a call the browser cannot answer.
 * @param bridge - the bridge the tools drive.
 * @returns the tool definitions, in catalogue order.
 */
export function buildBrowserTools(bridge: BrowserTarget) {
  return [
    defineTool({
