...
A CLI that cannot prompt must also be told to permit the tools. `agy` auto-denies anything needing approval in a headless run, and its `permissions.allow` rules are not consulted for MCP — `mcp(*)`, `mcp(<server>)`, `mcp(<server>/<tool>)` and `autoApproveTools` were all tried and all denied. The only switch that admits them is `--dangerously-skip-permissions`, which `agy-bridge.mjs` passes while the browser tools are enabled. `opencode` takes its MCP servers from `opencode.jsonc` instead of a subcommand and needs no such grant: with `--auto`, which its bridge already passes, it calls them directly. `opencode mcp list` reports each server's connection state, which is the fastest way to tell a registration problem from a permission one.
...
- **opencode prefers a project-local `opencode.jsonc`** over the global one, read from the working directory the CLI is spawned in. The bridge inherits `/app`, which carries no such file, so the global registration applies — but adding one to the repository root would silently shadow it, and the symptom is a model reporting it has no browser tools.
