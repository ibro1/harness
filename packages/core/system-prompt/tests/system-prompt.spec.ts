 ...
 */
const BUILT_IN = [
  'harness:identity', 'harness:turn-contract', 'deployment:persona-prefix', 'deployment:persona-suffix',
]
...
const SECTION_ORDER_NAMES = [
  'HARNESS_IDENTITY', 'HARNESS_TURN_CONTRACT', 'DEPLOYMENT_PERSONA_PREFIX',
...
describe('SystemPrompt', () => {
  ...
  it('keeps reusable instructions identical across local environments', async () => {
    ...
    try {
      ...
      const prefix = [IDENTITY, TURN_CONTRACT, 'Model model-a.', ...reusable].join('\n\n') + '\n\n'
  ...
  describe('built-in sections', () => {
    ...
    it('can omit the harness identity for a deployment that owns the complete persona', async () => {
      ...
      expect(assembly.sections.map(section => section.name))
        .toEqual(['harness:turn-contract', 'deployment:persona-prefix', 'deployment:persona-suffix'])
  ...
  it('assembles sections in order with context-resolved text and collected tools', async () => {
    ...
    const assembly = await ctx.systemPrompt.assemble()
    expect(assembly.sections.map(s => s.name)).toEqual([
      'harness:identity', 'harness:turn-contract', 'deployment:persona-prefix',
      'rules', 'cwd', 'deployment:persona-suffix',
    ])
    expect(assembly.sections.map(s => s.text)).toEqual([
      IDENTITY, TURN_CONTRACT, 'You are DeepSeek Harness.', 'Be precise.', 'cwd: /tmp', '',
    ])
    expect(assembly.contexts).toEqual([
  ...
  it('composes multiple system-prompt/assemble waterfall listeners in order, with the context', async () => {
    ...
    const ordered = [
      'harness:identity', 'harness:turn-contract', 'deployment:persona-prefix',
      'base', 'deployment:persona-suffix', 'from-a',
    ]
    expect(seen).toEqual([ordered])
    expect(assembly.sections.map(s => s.name)).toEqual(ordered)
  ...
  it('assembles snapshots so one-step mutations do not leak into future assemblies', async () => {
    ...
    expect(second.sections.map(section => section.name)).toEqual([
      'harness:identity', 'harness:turn-contract', 'deployment:persona-prefix',
      'base', 'deployment:persona-suffix',
    ])
  ...
  describe('prompt variables', () => {
    ...
    it.each([
      ...
      try {
        ...
        expect(renderPrompt(await ctx.systemPrompt.assemble()))
          .toBe(complete ? text : `${TURN_CONTRACT}\n\nactual-model\n\n${text}`)
      } finally {
        await ctx.fiber.dispose()
      }
    })
