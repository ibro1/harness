...
def run_render(args: argparse.Namespace) -> Path:
    """Do the render described by args; returns the output path."""
...
def _notify_target() -> tuple[str, str] | None:
    """(DSH_NOTIFY_URL, DSH_SESSION_ID) when both are set — the harness
    background-completion channel. None locally, so renders stay foreground."""
    url = os.environ.get("DSH_NOTIFY_URL", "").strip()
    session = os.environ.get("DSH_SESSION_ID", "").strip()
    return (url, session) if url and session else None


def _post_notify(url: str, session: str, label: str, code: int) -> None:
    """Best-effort ping so the session wakes; never masks the render result."""
    full = (f"{url}&session={urllib.parse.quote(session)}"
            f"&label={urllib.parse.quote(label)}&exit={code}")
    try:
        urllib.request.urlopen(urllib.request.Request(full, method="POST", data=b""), timeout=15).read()
    except Exception as error:
        print(f"notify failed: {error}", file=sys.stderr)


def _dispatch_background(cli_args: list[str], edit_dir: Path, label: str) -> None:
    """Re-exec this render detached so the caller's turn is not blocked. The
    child carries --_child: it renders in the foreground and fires the notify
    hook when it finishes."""
    bg_dir = edit_dir / ".bg"
    bg_dir.mkdir(parents=True, exist_ok=True)
    log = bg_dir / f"{label}-{int(time.time())}.log"
    child = [sys.executable, str(Path(__file__).resolve()), *cli_args, "--_child"]
    with open(log, "wb") as logf:
        subprocess.Popen(
            child, stdout=logf, stderr=subprocess.STDOUT,
            stdin=subprocess.DEVNULL, start_new_session=True,
        )
    print(f"Render started in the background (job {log.stem}).")
    print("End your turn and do NOT poll — the session is woken when it finishes.")
...
def main() -> None:
    ...
    ap.add_argument(
        "--foreground",
        action="store_true",
        help="Render inline even under the harness. Default: a FINAL render "
             "auto-backgrounds and notifies the session on completion, so the "
             "turn is not blocked (previews and drafts always run inline).",
    )
    ...
    # Under the harness, a FINAL render auto-backgrounds so a multi-minute job
    # never blocks the turn; previews/drafts stay inline for a tight QC loop,
    # and --foreground opts out. The detached worker runs with --_child.
    if (target is not None and not args._child and not args.foreground
            and not args.preview and not args.draft):
        _dispatch_background(sys.argv[1:], args.edl.resolve().parent, "render")
        return

    if args._child and target is not None:
        code = 0
        try:
            run_render(args)
        except SystemExit as exit_err:
            code = 0 if exit_err.code in (0, None) else 1
            if code:
                print(f"render exited: {exit_err.code}", file=sys.stderr)
        except Exception as error:
            code = 1
            print(f"render failed: {error}", file=sys.stderr)
        _post_notify(target[0], target[1], "render", code)
        sys.exit(code)
