...
- `GEMINI_API_KEY` resolves (only if using Gemini TTS via `speak.py`) — same `.env`/env resolution. A real key starts `AIzaSy` and is 39 chars; a key starting `AQ.` is a short-lived AI Studio web-session token that expires within hours — do not build a pipeline on one. Note: Gemini's free tier uses prompts/outputs for product improvement — surface that to the user before anything ships to a paying client.
...
- **`download.py <url> [--mode video|audio|transcript] [-o <dir>]`** — fetch a source from a URL (YouTube et al.) to edit; default lands in `<cwd>/edit/downloads`. YouTube bot-gates datacenter IPs, so it needs fresh account cookies — resolved from `$YT_COOKIES_FILE`, else fetched from `$YT_COOKIES_URL` with `Authorization: Bearer $YT_COOKIES_TOKEN` (the DeerFlow `/yt/cookies.txt` endpoint), else the local dev jar. It uses the only combination that works: cookies + `--js-runtimes node` + `player_client=mweb` (no bgutil, no WARP; `mweb` is the only client that works with cookies). `--mode transcript` pulls auto-subs (VTT) with no media — use it to analyse a video cheaply before deciding to edit. Cookies must be fresh (age-based staleness); the endpoint re-exports continuously.
- **`speak.py "<text>"`** — TTS / voiceover, provider-agnostic (`--provider gemini|elevenlabs`, Gemini default), cached, writes a mono 16-bit WAV *stem*, prints its duration. Gemini has **no accent/voice-style/speed parameter** — accent, tone and PACE come from `--style` (a natural-language instruction prefixed to the text), e.g. `--style "Read aloud in a warm Nigerian accent, brisk advertising pace — no dawdling."`. Gemini reads ~40% slow without a pace note. `--normalize` (loudnorm to −18 LUFS — generated lines vary up to ~10 dB), `--verify` (round-trip through Scribe to catch mangled words). **Audio-led:** you cannot set a duration — generate the voice, read the printed duration, then cut picture to fit; if a line overruns, shorten the copy and regenerate, never time-stretch. Needs `GEMINI_API_KEY` (or `ELEVENLABS_API_KEY`). Cached on a hash of (provider, model, voice, style, text). Gemini is the default and the right choice for an accented read (Nigerian, etc.) — the accent comes from `--style`, there is no accent voice to pick. Generate voiceovers ONLY through this helper (so provider, caching and normalization stay consistent); do not curl a TTS API directly. To force one, pass `--provider gemini` or `--provider elevenlabs`. A single voiceover is quick — run it in the FOREGROUND and let it block until the WAV is written; never background it and poll (you will spin emitting "waiting…"). For a genuinely long batch, use the background-notify pattern under "Long jobs" below instead of polling. Generate the ONE voiceover the edit needs; do not synthesize a matrix of voices/accents to "compare" unless the user asked — on Gemini's per-minute-rate-limited free tier a batch turns a 10s job into many minutes of 429 backoff.
...
Then END your turn, telling the user the job is running and you will report back — do not poll. When it finishes, the route wakes this session with the result and the download links, and you relay them to the user. `$DSH_NOTIFY_URL` (already carries its token) and `$DSH_SESSION_ID` are in your environment. If either is unset — a native model rather than the agy/opencode CLIs — that channel is unavailable, so run the job in the foreground instead.
