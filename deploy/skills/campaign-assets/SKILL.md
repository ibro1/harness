...
**Under the harness, publish each finished asset with the `publish_output`
tool.** Render wherever you like, then publish. A file sitting in the workspace
is invisible to the person who asked for it — they get a path into a container
instead of something they can open — and publishing is what puts it in the
session's outputs drawer. Publishing is the whole of delivery; do not also copy
the file into `edit/` or anywhere else afterwards.
