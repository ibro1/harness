#!/usr/bin/env node
// An MCP server exposing the harness's WhatsApp tools to a CLI that runs its own
...
const COMMAND_URL = process.env.WA_COMMAND_URL ?? 'http://127.0.0.1:3081/whatsapp/command'
  ...
  switch (request.method) {
    case 'initialize':
      return {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'dsh-whatsapp', version: '1.0.0' },
      }
    case 'tools/list':
...
if (TOKEN === '') warn('WA_AGENT_TOKEN is unset; every call will be refused')
