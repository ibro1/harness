...
import * as ToolSocial from '@deepseek-ai/dsh-tool-social'
...
const TOOL_PACKAGES: ToolPackage[] = [
  ...
  {
    pkg: '@deepseek-ai/dsh-tool-social',
    dir: 'tool-social',
    source: 'packages/social/tool-social/src/index.ts',
    requires: ['ctx.tools', 'ctx.social', 'ctx.webServer'],
    writes: ['tool/call', 'tool/result'],
    async mount(ctx) {
      // Schema harvest never reaches a provider or a route: the seam answers an
      // empty catalog and the web realm records the registration and nothing else.
      ctx.provide('social', {
        targets: () => Promise.resolve([]),
        post: () => Promise.reject(new Error('gen-tool-catalog: publishing is unreachable during schema harvest')),
      } as never)
      ctx.provide('webServer', { register: () => () => {} } as never)
      await ctx.plugin(ToolSocial)
    },
    note:
      'social_post asks a human through the approval seam inside the executing operation, so no caller reaches a publication that skipped it.',
  },
]
