...
<a id="ctxbrowserbridge--browserbridge"></a>

### `ctx.browserBridge` — `BrowserBridge`

The connected browser, and the request/reply channel to it.

One connection at a time: a second extension replaces the first, because two browsers answering the same `browser_click` would make the result ambiguous rather than doubling the capability.
...
Source: [`packages/host/browser-bridge/src/index.ts`](../../packages/host/browser-bridge/src/index.ts)
